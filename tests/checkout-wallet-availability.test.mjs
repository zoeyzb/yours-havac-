...
test('Stripe wallet availability resolves initially and on later changes', () => {
  ...
  assert.ok(checkoutForm.includes('event?.availablePaymentMethods?.applePay === true'))
  assert.ok(checkoutForm.includes('apple.on("availablepaymentmethodschange"'))
  assert.ok(checkoutForm.includes('event?.paymentMethods?.applePay?.available === true'))
  assert.ok(checkoutForm.includes('el.on("ready"'))
  assert.ok(checkoutForm.includes('event?.availablePaymentMethods?.googlePay === true'))
  assert.ok(checkoutForm.includes('event?.paymentMethods?.googlePay?.available === true'))
