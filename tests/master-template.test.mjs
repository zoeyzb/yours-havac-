...
test('checkout stays onsite and uses a custom Stripe Elements handoff', () => {
  ...
  assert.ok(checkoutForm.includes('googlePay: "always"'), 'Google Pay should use a native Express Checkout wallet block')
  assert.ok(checkoutForm.includes('fast-pay-grid'), 'native wallets must mount inside a visible measurable container')
  assert.ok(checkoutForm.includes('buttonHeight: 55'), 'Express Checkout wallet height must stay within Stripe\'s 40-55px limit')
  assert.ok(checkoutForm.includes('buttonType: { applePay: "plain" }') && checkoutForm.includes('buttonType: { googlePay: "pay" }'), 'wallets should use Stripe\'s native Apple Pay and Google Pay branding')
  ...
  assert.ok(checkoutForm.includes('hosted("klarna")'), 'Klarna should open its real hosted payment session')
  assert.ok(checkoutForm.indexOf('card.mount(cardRef.current)') < checkoutForm.indexOf('appleElements.create("expressCheckout"'), 'card checkout should mount before optional wallet UI')
...
test('checkout visual polish keeps proof people, four-step flow, and orange CTA', () => {
  ...
  assert.equal(checkoutForm.includes('fast-pay-apple--hidden'), false, 'Stripe wallet elements must never mount inside display none')
...
test('checkout speed and depth pass keep the payment surface fast and dimensional', () => {
  ...
  assert.ok(checkoutForm.includes('Promise.all([') && checkoutForm.includes('ensureStripeJs(),') && checkoutForm.includes('createIntent("card")'), 'card intent should preload in parallel with Stripe.js')
