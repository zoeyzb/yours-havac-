...
test('checkout stays onsite and uses a custom Stripe Elements handoff', () => {
  ...
  assert.ok(checkoutForm.includes('openHosted("wallet")'), 'restricted in-app browsers should fall back to real hosted wallet checkout')
  assert.ok(checkoutForm.includes('wallet-checkout-button--apple'), 'Apple Pay must remain visible when a webview hides native wallets')
  assert.ok(checkoutForm.includes('wallet-checkout-button--google'), 'Google Pay must remain visible when a webview hides native wallets')
